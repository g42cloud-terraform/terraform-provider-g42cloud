...
func TestAccLBV2LoadBalancer_basic(t *testing.T) {
	...
	resourceName := "g42cloud_lb_loadbalancer.loadbalancer_1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SecGroupExists(
						"g42cloud_networking_secgroup.secgroup_1", &sg_1),
					testAccCheckNetworkingV2SecGroupExists(
						"g42cloud_networking_secgroup.secgroup_1", &sg_2),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SecGroupExists(
						"g42cloud_networking_secgroup.secgroup_2", &sg_1),
					testAccCheckNetworkingV2SecGroupExists(
						"g42cloud_networking_secgroup.secgroup_2", &sg_2),
					testAccCheckLBV2LoadBalancerHasSecGroup(&lb, &sg_1),
...
func testAccCheckLBV2LoadBalancerDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_lb_loadbalancer" {
			continue
		}
...
resource "g42cloud_lb_loadbalancer" "loadbalancer_1" {
  ...
  vip_subnet_id      = data.g42cloud_vpc_subnet.test.subnet_id
  security_group_ids = [
    g42cloud_networking_secgroup.secgroup_1.id
  ]
}
...
resource "g42cloud_lb_loadbalancer" "loadbalancer_1" {
  ...
  security_group_ids = [
    g42cloud_networking_secgroup.secgroup_1.id,
    g42cloud_networking_secgroup.secgroup_2.id
  ]
}
`, rNameSecg1, rNameSecg2, rName)
}
