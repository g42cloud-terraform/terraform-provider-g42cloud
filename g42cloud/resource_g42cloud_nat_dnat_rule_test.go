...
func TestAccNatDnat_basic(t *testing.T) {
	...
	resourceName := "g42cloud_nat_dnat_rule.dnat"
...
func testAccCheckNatDnatDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_nat_dnat_rule" {
			continue
		}
		...
		if err == nil {
			return fmt.Errorf("g42cloud dnat rule still exists at %s", url)
		}
	}
...
func testAccCheckNatDnatExists() resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		rs, ok := s.RootModule().Resources["g42cloud_nat_dnat_rule.dnat"]
		if !ok {
			return fmt.Errorf("Error checking g42cloud_nat_dnat_rule.dnat exist, err=not found g42cloud_nat_dnat_rule.dnat")
		}

		url, err := replaceVarsForTest(rs, "dnat_rules/{id}")
		if err != nil {
			return fmt.Errorf("Error checking g42cloud_nat_dnat_rule.dnat exist, err=building url failed: %s", err)
		}
		url = client.ServiceURL(url)
		...
		if err != nil {
			if _, ok := err.(golangsdk.ErrDefault404); ok {
				return fmt.Errorf("g42cloud_nat_dnat_rule.dnat is not exist")
			}
			return fmt.Errorf("Error checking g42cloud_nat_dnat_rule.dnat exist, err=send request failed: %s", err)
		}
		return nil
	}
}
...
resource "g42cloud_nat_dnat_rule" "dnat" {
  nat_gateway_id = g42cloud_nat_gateway.nat_1.id
  floating_ip_id = g42cloud_vpc_eip.eip_1.id
  private_ip     = g42cloud_compute_instance.instance_1.network.0.fixed_ip_v4
  protocol       = "tcp"
