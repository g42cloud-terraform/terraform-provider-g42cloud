...
func TestAccFgsV2Function_basic(t *testing.T) {
	...
	resourceName := "g42cloud_fgs_function.test"
...
func testAccCheckFgsV2FunctionDestroy(s *terraform.State) error {
	...
	fgsClient, err := config.FgsV2Client(G42_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42Cloud FGS V2 client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_fgs_function" {
			continue
		}
...
`, rName, G42_ENTERPRISE_PROJECT_ID_TEST)
}
