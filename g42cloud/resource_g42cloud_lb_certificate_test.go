...
func TestAccLBV2Certificate_basic(t *testing.T) {
	...
	resourceName := "g42cloud_lb_certificate.certificate_1"
...
func TestAccLBV2Certificate_client(t *testing.T) {
	var c certificates.Certificate
	name := fmt.Sprintf("tf-cert-%s", acctest.RandString(5))
	resourceName := "g42cloud_lb_certificate.certificate_client"
...
func testAccCheckLBV2CertificateDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_lb_certificate" {
			continue
		}
...
func testAccLBV2CertificateConfig_client(name string) string {
	return fmt.Sprintf(`
resource "g42cloud_lb_certificate" "certificate_client" {
  name        = "%s"
