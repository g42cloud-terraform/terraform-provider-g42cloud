...
func TestAccBandWidthDataSource_basic(t *testing.T) {
	...
	dataName := "data.g42cloud_vpc_bandwidth.test"
...
func testAccCheckBandWidthDataSourceExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		bandwidthRs, ok := s.RootModule().Resources["g42cloud_vpc_bandwidth.test"]
		if !ok {
			return fmt.Errorf("can't find g42cloud_vpc_bandwidth.test in state")
		}
...
data "g42cloud_vpc_bandwidth" "test" {
  name = g42cloud_vpc_bandwidth.test.name
}
`, rName)
}
