		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2SecGroupExists(
						"g42cloud_networking_secgroup.secgroup_1", &secgroup_1),
					testAccCheckNetworkingV2SecGroupExists(
						"g42cloud_networking_secgroup.secgroup_2", &secgroup_2),
					testAccCheckNetworkingV2SecGroupRuleExists(
						"g42cloud_networking_secgroup_rule.secgroup_rule_1", &secgroup_rule_1),
					testAccCheckNetworkingV2SecGroupRuleExists(
						"g42cloud_networking_secgroup_rule.secgroup_rule_2", &secgroup_rule_2),
				),
			},
			{
				ResourceName:      "g42cloud_networking_secgroup_rule.secgroup_rule_1",
				ImportState:       true,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"g42cloud_networking_secgroup_rule.secgroup_rule_1", "remote_ip_prefix", "2001:558:FC00::/39"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SecGroupRuleExists(
						"g42cloud_networking_secgroup_rule.secgroup_rule_1", &secgroup_rule_1),
					resource.TestCheckResourceAttr(
						"g42cloud_networking_secgroup_rule.secgroup_rule_1", "protocol", "115"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2SecGroupRuleDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_networking_secgroup_rule" {
			continue
		}
...
resource "g42cloud_networking_secgroup" "secgroup_2" {
  name = "secgroup_2"
  description = "terraform security group rule acceptance test"
}
...
resource "g42cloud_networking_secgroup_rule" "secgroup_rule_2" {
  direction = "ingress"
  ethertype = "IPv4"
  port_range_max = 80
  port_range_min = 80
  protocol = "tcp"
  remote_group_id = "${g42cloud_networking_secgroup.secgroup_1.id}"
  security_group_id = "${g42cloud_networking_secgroup.secgroup_2.id}"
}
`
