...
func TestAccKmsKey_Basic(t *testing.T) {
	...
	var resourceName = "g42cloud_kms_key.key_1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "region", G42_REGION_NAME),
				),
			},
			{
...
func testAccCheckKmsKeyDestroy(s *terraform.State) error {
	...
	kmsClient, err := config.KmsKeyV1Client(G42_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42Cloud kms client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_kms_key" {
			continue
		}
