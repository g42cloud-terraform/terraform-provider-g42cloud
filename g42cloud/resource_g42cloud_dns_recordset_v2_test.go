		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDNSV2RecordSetExists("g42cloud_dns_recordset.recordset_1", &recordset),
					resource.TestCheckResourceAttr(
						"g42cloud_dns_recordset.recordset_1", "description", "a record set"),
					resource.TestCheckResourceAttr(
						"g42cloud_dns_recordset.recordset_1", "records.0", "10.1.0.0"),
				),
			},
			{
				ResourceName:      "g42cloud_dns_recordset.recordset_1",
				ImportState:       true,
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("g42cloud_dns_recordset.recordset_1", "name", zoneName),
					resource.TestCheckResourceAttr("g42cloud_dns_recordset.recordset_1", "ttl", "6000"),
					resource.TestCheckResourceAttr("g42cloud_dns_recordset.recordset_1", "type", "A"),
					resource.TestCheckResourceAttr(
						"g42cloud_dns_recordset.recordset_1", "description", "an updated record set"),
					resource.TestCheckResourceAttr(
						"g42cloud_dns_recordset.recordset_1", "records.0", "10.1.0.1"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestMatchResourceAttr(
						"g42cloud_dns_recordset.recordset_1", "ttl", regexp.MustCompile("^[0-9]+$")),
				),
			},
		},
	})
...
func testAccCheckDNSV2RecordSetDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_dns_recordset" {
			continue
		}
...
func testAccDNSV2RecordSet_basic(zoneName string) string {
	return fmt.Sprintf(`
		...
		resource "g42cloud_dns_recordset" "recordset_1" {
