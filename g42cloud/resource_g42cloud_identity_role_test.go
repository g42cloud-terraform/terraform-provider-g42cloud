...
func TestAccIdentityRole_basic(t *testing.T) {
	...
	resourceName := "g42cloud_identity_role.test"
...
func testAccCheckIdentityRoleDestroy(s *terraform.State) error {
	...
	identityClient, err := config.IAMV3Client(G42_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42Cloud identity client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_identity_role" {
			continue
		}
...
func testAccIdentityRole_basic(roleName string) string {
	return fmt.Sprintf(`
resource "g42cloud_identity_role" test {
  name        = "%s"
