		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIdentityV3UserExists("g42cloud_identity_user_v3.user_1", &user),
					resource.TestCheckResourceAttrPtr(
						"g42cloud_identity_user_v3.user_1", "name", &user.Name),
					resource.TestCheckResourceAttr(
						"g42cloud_identity_user_v3.user_1", "enabled", "true"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPtr(
						"g42cloud_identity_user_v3.user_1", "name", &user.Name),
					resource.TestCheckResourceAttr(
						"g42cloud_identity_user_v3.user_1", "enabled", "false"),
				),
			},
		},
	})
...
func testAccCheckIdentityV3UserDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_identity_user_v3" {
			continue
		}
