		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkingV2SecGroupExists(
						"g42cloud_networking_secgroup.secgroup_1", &security_group),
					testAccCheckNetworkingV2SecGroupRuleCount(&security_group, 4),
				),
			},
			{
				ResourceName:      "g42cloud_networking_secgroup.secgroup_1",
				ImportState:       true,
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrPtr(
						"g42cloud_networking_secgroup.secgroup_1", "id", &security_group.ID),
					resource.TestCheckResourceAttr(
						"g42cloud_networking_secgroup.secgroup_1", "name", "security_group_2"),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2SecGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_networking_secgroup" {
			continue
		}
