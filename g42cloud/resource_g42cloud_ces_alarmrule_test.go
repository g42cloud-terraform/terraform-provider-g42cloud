...
func TestAccCESAlarmRule_basic(t *testing.T) {
	...
	resourceName := "g42cloud_ces_alarmrule.alarmrule_1"
...
func testCESAlarmRuleDestroy(s *terraform.State) error {
	...
	networkingClient, err := config.CesV1Client(G42_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42Cloud ces client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_ces_alarmrule" {
			continue
		}
