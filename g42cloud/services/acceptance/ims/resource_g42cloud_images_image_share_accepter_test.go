...
func TestAccImsImageShareAccepter_basic(t *testing.T) {
	...
	rName := "g42cloud_images_image_share_accepter.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "image_id", acceptance.G42_IMAGE_SHARE_SOURCE_IMAGE_ID),
				),
			},
		},
	})
...
func testImsImageShareAccepter_basic() string {
	return fmt.Sprintf(`
resource "g42cloud_images_image_share_accepter" "test" {
  image_id = "%s"
}
`, acceptance.G42_IMAGE_SHARE_SOURCE_IMAGE_ID)
}
