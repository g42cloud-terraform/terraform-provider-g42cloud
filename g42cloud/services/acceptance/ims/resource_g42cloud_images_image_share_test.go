...
func TestAccImsImageShare_basic(t *testing.T) {
	...
	rName := "g42cloud_images_image_share.test"
...
resource "g42cloud_images_image_share" "test" {
  source_image_id    = g42cloud_images_image.test.id
  target_project_ids = ["%[2]s"]
}
`, testAccImsImage_basic(imageName), acceptance.G42_DEST_PROJECT_ID)
}
...
`, testAccImsImage_basic(imageName), acceptance.G42_DEST_PROJECT_ID_TEST)
}
