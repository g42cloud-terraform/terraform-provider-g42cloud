...
func getResourceSMNLogtankFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := cfg.SmnV2Client(acceptance.G42_REGION_NAME)
	if err != nil {
...
func TestAccSMNV2Logtank_basic(t *testing.T) {
	var (
		...
		logtankResourceName = fmt.Sprintf("g42cloud_smn_logtank.%s", rName)
	)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(logtankResourceName, "topic_urn",
						"g42cloud_smn_topic.test", "topic_urn"),
...
resource "g42cloud_smn_logtank" "%[1]s" {
  topic_urn      = g42cloud_smn_topic.test.topic_urn
...
resource "g42cloud_lts_group" "test_update" {
  group_name  = "%[1]s_update"
  ttl_in_days = 1
}

resource "g42cloud_lts_stream" "test_update" {
  group_id    = g42cloud_lts_group.test_update.id
...
resource "g42cloud_smn_logtank" "%[1]s" {
  ...
  log_stream_id = g42cloud_lts_stream.test_update.id
}
`, rName)
}
