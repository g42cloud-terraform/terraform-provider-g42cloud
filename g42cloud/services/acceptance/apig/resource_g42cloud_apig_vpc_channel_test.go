...
func TestAccApigVpcChannelV2_basic(t *testing.T) {
	var (
		...
		resourceName = "g42cloud_apig_vpc_channel.test"
...
func testAccCheckApigVpcChannelDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating G42Cloud APIG v2 client: %s", err)
	}
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_apig_vpc_channel" {
			continue
		}
...
resource "g42cloud_compute_instance" "newone" {
  name               = "%s"
  image_id           = data.g42cloud_images_image.test.id
  flavor_id          = data.g42cloud_compute_flavors.test.ids[0]
  security_group_ids = [g42cloud_networking_secgroup.test.id]
...
resource "g42cloud_apig_vpc_channel" "test" {
  ...
  members {
    id     = g42cloud_compute_instance.newone.id
    weight = 70
  }
}
...
resource "g42cloud_apig_vpc_channel" "test" {
  ...
  members {
    ip_address = g42cloud_vpc_eip.test.address
  }
}
...
resource "g42cloud_vpc_eip" "newone" {
  publicip {
    type = "5_bgp"
  }
...
resource "g42cloud_apig_vpc_channel" "test" {
  ...
  members {
    ip_address = g42cloud_vpc_eip.newone.address
    weight     = 70
  }
}
