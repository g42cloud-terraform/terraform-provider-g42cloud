...
func TestAccApigGroupV2_basic(t *testing.T) {
	var (
		...
		resourceName = "g42cloud_apig_group.test"
...
func testAccCheckApigGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_apig_group" {
			continue
		}
...
func testAccApigGroup_variablesBase(rName string) string {
	return fmt.Sprintf(`
resource "g42cloud_apig_environment" "test1" {
  name        = "%s_1"
  instance_id = g42cloud_apig_instance.test.id
  description = "Created by script"
}

resource "g42cloud_apig_environment" "test2" {
  name        = "%s_2"
  instance_id = g42cloud_apig_instance.test.id
  description = "Created by script"
}
...
resource "g42cloud_apig_group" "test" {
  ...
  environment {
    environment_id = g42cloud_apig_environment.test1.id
  ...
  environment {
    environment_id = g42cloud_apig_environment.test2.id
