...
func TestAccApigCustomAuthorizerV2_basic(t *testing.T) {
	var (
		...
		resourceName = "g42cloud_apig_custom_authorizer.test"
...
func testAccCheckApigCustomAuthorizerDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_apig_custom_authorizer" {
			continue
		}
...
resource "g42cloud_apig_custom_authorizer" "test" {
  instance_id  = g42cloud_apig_instance.test.id
  name         = "%s"
  function_urn = g42cloud_fgs_function.test.urn
  type         = "FRONTEND"
  is_body_send = true
  cache_age    = 60
