...
func TestAccThrottlingPolicyAssociate_basic(t *testing.T) {
	...
	resourceName := "g42cloud_apig_throttling_policy_associate.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "policy_id",
						"g42cloud_apig_throttling_policy.test", "id"),
...
resource "g42cloud_apig_throttling_policy" "test" {
  instance_id       = g42cloud_apig_instance.test.id
  name              = "%s"
  type              = "API-based"
  period            = 15000
...
resource "g42cloud_apig_throttling_policy_associate" "test" {
  instance_id = g42cloud_apig_instance.test.id
  policy_id   = g42cloud_apig_throttling_policy.test.id

  publish_ids = [
    g42cloud_apig_api_publishment.test[0].publish_id
  ]
}
...
resource "g42cloud_apig_throttling_policy_associate" "test" {
  ...
  publish_ids = [
    g42cloud_apig_api_publishment.test[1].publish_id
  ]
}
`, testAccThrottlingPolicyAssociate_base(rName))
}
