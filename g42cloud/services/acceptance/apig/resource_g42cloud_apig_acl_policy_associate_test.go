...
func TestAccAclPolicyAssociate_basic(t *testing.T) {
	var (
		...
		rName = "g42cloud_apig_acl_policy_associate.test"
	)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "policy_id",
						"g42cloud_apig_acl_policy.test", "id"),
...
resource "g42cloud_apig_acl_policy" "test" {
  instance_id = g42cloud_apig_instance.test.id
  name        = "%[2]s"
  type        = "PERMIT"
  entity_type = "IP"
...
resource "g42cloud_apig_acl_policy_associate" "test" {
  instance_id = g42cloud_apig_instance.test.id
  policy_id   = g42cloud_apig_acl_policy.test.id
