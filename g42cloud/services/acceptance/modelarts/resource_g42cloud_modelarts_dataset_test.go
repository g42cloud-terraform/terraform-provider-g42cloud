...
func getDatesetResourceFunc(config *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := config.ModelArtsV2Client(acceptance.G42_REGION_NAME)
	if err != nil {
...
func TestAccResourceDateset_basic(t *testing.T) {
	...
	resourceName := "g42cloud_modelarts_dataset.test"
...
resource "g42cloud_obs_bucket_object" "output" {
  bucket  = g42cloud_obs_bucket.bucket.bucket
  key     = "output/t2"
  content = "some_bucket_content"
}
...
resource "g42cloud_modelarts_dataset" "test" {
  name        = "%s"
  type        = 1
  output_path = "/${g42cloud_obs_bucket.bucket.bucket}/output/"
  description = "%s"
  data_source {
    path = "/${g42cloud_obs_bucket.bucket.bucket}/input/"
  }
  ...
  depends_on = [
    g42cloud_obs_bucket_object.input,
    g42cloud_obs_bucket_object.output
  ]
}
