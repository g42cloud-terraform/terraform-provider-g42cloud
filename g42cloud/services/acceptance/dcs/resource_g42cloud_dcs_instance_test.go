...
func getDcsResourceFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := c.DcsV2Client(acceptance.G42_REGION_NAME)
	if err != nil {
		return nil, fmt.Errorf("error creating G42cloud DCS client(V2): %s", err)
	}
...
resource "g42cloud_dcs_instance" "instance_1" {
  ...
  availability_zones = [data.g42cloud_availability_zones.test.names[0]]
...
}`, instanceName, acceptance.G42_ENTERPRISE_PROJECT_ID_TEST)
}
