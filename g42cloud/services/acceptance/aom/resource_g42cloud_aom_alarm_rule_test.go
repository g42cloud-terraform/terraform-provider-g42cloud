...
func getAlarmRuleResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	c, err := conf.HcAomV2Client(acceptance.G42_REGION_NAME)
	if err != nil {
...
func TestAccAOMAlarmRule_basic(t *testing.T) {
	...
	resourceName := "g42cloud_aom_alarm_rule.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "dimensions.0.value", "g42cloud_compute_instance.test", "id"),
...
resource "g42cloud_aom_alarm_rule" "test" {
  name                 = "%s"
  alarm_level          = 2
  ...
  dimensions {
    name  = "hostID"
    value = g42cloud_compute_instance.test.id
  }
