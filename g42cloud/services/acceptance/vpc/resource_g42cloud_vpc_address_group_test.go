...
func getVpcAddressGroupResourceFunc(conf *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := conf.HcVpcV3Client(acceptance.G42_REGION_NAME)
	if err != nil {
		return nil, fmt.Errorf("error creating G42cloud VPC client: %s", err)
	}
...
func TestAccVpcAddressGroup_basic(t *testing.T) {
	...
	resourceName := "g42cloud_vpc_address_group.test"
