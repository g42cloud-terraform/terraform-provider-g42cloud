...
func TestAccElbV3L7Rule_basic(t *testing.T) {
	...
	resourceName := "g42cloud_elb_l7rule.l7rule_1"
...
func testAccCheckElbV3L7RuleDestroy(s *terraform.State) error {
	...
	lbClient, err := config.ElbV3Client(acceptance.G42_REGION_NAME)
	if err != nil {
		return fmtp.Errorf("Error creating G42Cloud load balancing client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_elb_l7rule" {
			continue
		}
...
func testAccELBL7RuleImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		policy, ok := s.RootModule().Resources["g42cloud_elb_l7policy.test"]
		if !ok {
			return "", fmt.Errorf("policy not found: %s", policy)
		}
		rule, ok := s.RootModule().Resources["g42cloud_elb_l7rule.l7rule_1"]
		if !ok {
...
resource "g42cloud_elb_l7policy" "test" {
  name         = "%s"
  description  = "test description"
  listener_id  = g42cloud_elb_listener.test.id
  redirect_pool_id = g42cloud_elb_pool.test.id
}
...
resource "g42cloud_elb_l7rule" "l7rule_1" {
  l7policy_id  = g42cloud_elb_l7policy.test.id
  type         = "PATH"
