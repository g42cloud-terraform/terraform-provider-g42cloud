...
func TestAccElbV3Certificate_basic(t *testing.T) {
	...
	resourceName := "g42cloud_elb_certificate.test"
...
func testAccCheckElbV3CertificateDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_elb_certificate" {
			continue
		}
...
func testAccElbV3CertificateConfig_basic(name string) string {
	return fmt.Sprintf(`
resource "g42cloud_elb_certificate" "test" {
  name        = "%s"
...
`, name, acceptance.G42_ENTERPRISE_PROJECT_ID_TEST)
}
