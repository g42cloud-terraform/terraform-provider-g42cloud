...
func getELBResourceFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	if eipType != "" && eipID != "" {
		eipClient, err := c.NetworkingV1Client(acceptance.G42_REGION_NAME)
		if err != nil {
...
func TestAccElbV3LoadBalancer_basic(t *testing.T) {
	...
	resourceName := "g42cloud_elb_loadbalancer.test"
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(
						resourceName, "l4_flavor_id", "data.g42cloud_elb_flavors.l4flavors", "ids.0"),
					resource.TestCheckResourceAttrPair(
						resourceName, "l7_flavor_id", "data.g42cloud_elb_flavors.l7flavors", "ids.0"),
				),
			},
		},
	})
