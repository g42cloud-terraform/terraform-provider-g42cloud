...
func TestAccPrivateDnatRule_basic(t *testing.T) {
	var (
		...
		rName = "g42cloud_nat_private_dnat_rule.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "backend_interface_id",
						"g42cloud_compute_instance.test", "network.0.port"),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "description", "Created by acc test"),
					resource.TestCheckResourceAttrPair(rName, "backend_interface_id",
						"data.g42cloud_networking_port.test", "id"),
					resource.TestCheckResourceAttr(rName, "internal_service_port", "2000"),
				),
			},
			{
...
resource "g42cloud_network_acl" "test" {
  name = "%[3]s"
  ...
  inbound_rules = [
    g42cloud_network_acl_rule.test.id
  ]
}

resource "g42cloud_network_acl_rule" "test" {
  name                   = "%[3]s"
  protocol               = "tcp"
  action                 = "allow"
  source_ip_address      = g42cloud_vpc.test.cidr
  source_port            = "8080"
  destination_ip_address = "0.0.0.0/0"
  destination_port       = "8081"
}

resource "g42cloud_networking_secgroup_rule" "in_v4_icmp_all" {
...
resource "g42cloud_networking_secgroup_rule" "in_v4_elb_member" {
  security_group_id = g42cloud_networking_secgroup.test.id
  ethertype         = "IPv4"
  direction         = "ingress"
  protocol          = "tcp"
  ports             = "80,8081"
  remote_ip_prefix  = g42cloud_vpc.test.cidr
}

resource "g42cloud_networking_secgroup_rule" "in_v4_all_group" {
  security_group_id = g42cloud_networking_secgroup.test.id
  ethertype         = "IPv4"
  direction         = "ingress"
  remote_group_id   = g42cloud_networking_secgroup.test.id
}

resource "g42cloud_networking_secgroup_rule" "out_v4_all" {
  security_group_id = g42cloud_networking_secgroup.test.id
  ethertype         = "IPv4"
  direction         = "egress"
  remote_ip_prefix  = "0.0.0.0/0"
...
resource "g42cloud_compute_eip_associate" "test" {
...
resource "g42cloud_elb_loadbalancer" "test" {
  ...
  ipv4_subnet_id = g42cloud_vpc_subnet.test.subnet_id
...
resource "g42cloud_elb_monitor" "test" {
  protocol    = "HTTP"
  interval    = 20
  timeout     = 15
  max_retries = 10
...
resource "g42cloud_elb_member" "test" {
  address       = g42cloud_compute_instance.test.access_ip_v4
  protocol_port = 8080
  pool_id       = g42cloud_elb_pool.test.id
  subnet_id     = g42cloud_vpc_subnet.test.subnet_id
}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(rName, "description", "Created by acc test"),
					resource.TestCheckResourceAttrPair(rName, "backend_interface_id",
						"g42cloud_networking_vip.test", "id"),
					resource.TestCheckResourceAttr(rName, "internal_service_port", "2000"),
				),
			},
			{
