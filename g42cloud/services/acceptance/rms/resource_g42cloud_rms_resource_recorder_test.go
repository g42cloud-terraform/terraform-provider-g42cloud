...
func TestAccRecorder_basic(t *testing.T) {
	...
	rName := "g42cloud_rms_resource_recorder.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(rName, "obs_channel.0.bucket",
						"g42cloud_obs_bucket.test", "bucket"),
				),
			},
			{
...
resource "g42cloud_rms_resource_recorder" "test" {
  ...
  obs_channel {
    bucket = g42cloud_obs_bucket.test.id
    region = "%s"
  }
}
`, testRecorder_base(name), acceptance.G42_REGION_NAME)
}
