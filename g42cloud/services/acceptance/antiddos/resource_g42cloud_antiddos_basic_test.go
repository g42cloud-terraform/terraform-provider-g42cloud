...
func TestAccAntiDdos_basic(t *testing.T) {
	...
	resourceName := "g42cloud_antiddos_basic.antiddos_1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "public_ip", "g42cloud_vpc_eip.eip_1", "address"),
				),
			},
			{
...
func testAccCheckAntiDdosExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		client, err := cfg.AntiDDosV1Client(acceptance.G42_REGION_NAME)
		if err != nil {
...
resource "g42cloud_antiddos_basic" "antiddos_1" {
  eip_id            = g42cloud_vpc_eip.eip_1.id
  traffic_threshold = %d
}
`, rName, threshold)
}
