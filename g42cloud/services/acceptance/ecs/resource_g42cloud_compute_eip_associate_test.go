...
func TestAccComputeEIPAssociate_basic(t *testing.T) {
	...
	resourceName := "g42cloud_compute_eip_associate.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair("data.g42cloud_compute_instance.test", "public_ip",
						"g42cloud_vpc_eip.test", "address"),
				),
			},
			{
...
func testAccCheckComputeEIPAssociateDestroy(s *terraform.State) error {
	...
	computeClient, err := cfg.ComputeV1Client(acceptance.G42_REGION_NAME)
	if err != nil {
		return fmt.Errorf("error creating compute client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42cloud_compute_eip_associate" {
			continue
		}
...
func testAccCheckVpcV1EIPExists(n string, eip *eips.PublicIp) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		networkingClient, err := config.NetworkingV1Client(acceptance.G42_REGION_NAME)
		if err != nil {
...
data "g42cloud_compute_instance" "test" {
  depends_on = [g42cloud_compute_eip_associate.test]
...
resource "g42cloud_compute_eip_associate" "test" {
  public_ip   = g42cloud_vpc_eip.test.address
  instance_id = g42cloud_compute_instance.test.id
  fixed_ip    = g42cloud_compute_instance.test.access_ip_v4
}
`, testAccComputeEIPAssociate_Base(rName))
}
...
func TestAccComputeEIPAssociate_bandwidth(t *testing.T) {
	...
	resourceName := "g42cloud_compute_eip_associate.test"
	bwResourceName := "g42cloud_vpc_bandwidth.bandwidth_1"
	ecsResourceName := "g42cloud_compute_instance.test"
...
data "g42cloud_images_image" "image_1" {
...
resource "g42cloud_compute_instance" "test" {
  name               = "%[1]s"
  image_id           = data.g42cloud_images_image.image_1.id
  availability_zone  = data.g42cloud_availability_zones.test.names[0]
  flavor_id          = "c6.large.2"
  security_group_ids = [g42cloud_networking_secgroup.test.id]

  network {
    uuid        = g42cloud_vpc_subnet.subnet_1.id
    ipv6_enable = true
  }
}
...
resource "g42cloud_compute_eip_associate" "test" {
  bandwidth_id = g42cloud_vpc_bandwidth.bandwidth_1.id
  instance_id  = g42cloud_compute_instance.test.id
  fixed_ip     = g42cloud_compute_instance.test.access_ip_v6
}
`, rName)
}
