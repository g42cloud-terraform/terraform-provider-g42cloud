...
Manages a DLI global variable resource within G42Cloud.

## Example Usage

```hcl
resource "g42cloud_dli_global_variable" "test" {
  name  = "demo"
  value = "abc"
}
```
...
* `is_sensitive` - (Optional, Bool, ForceNew) Whether to set a variable as a sensitive variable. The default value
  is **false**.
...
```bash
$ terraform import g42cloud_dli_global_variable.test demo_name
```
